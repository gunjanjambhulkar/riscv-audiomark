...
//#if !defined(__riscv) || !defined(__riscv_vector) || (__riscv_v_intrinsic < 1000000)
    ...
    vint16m1_t va16 = __riscv_vle16_v_i16m1(a, vl);
    vint16m1_t vb16 = __riscv_vle16_v_i16m1(b, vl);

    // Widen a to i32
    vint32m2_t vacc = __riscv_vwadd_vx_i32m2(va16, 0, vl);

    // Widen multiply: (int32)alpha * (int32)b
    vint32m2_t vprod = __riscv_vwmul_vx_i32m2(vb16, alpha, vl);

    // acc = a + alpha*b (in i32)
    vacc = __riscv_vadd_vv_i32m2(vacc, vprod, vl);

    // Saturate/narrow i32 -> i16 with shift 0 (no rounding issues)
    vint16m1_t vy16 = __riscv_vnclip_wx_i16m1(vacc, 0, __RISCV_VXRM_RNU, vl);

    __riscv_vse16_v_i16m1(y, vy16, vl);
